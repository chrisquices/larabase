<x-backend.app-layout>
    <x-slot name="title">{{ __('backend.create_dogs') }}</x-slot>

    <x-backend.heading>
        <x-backend.card.title>{{ __('backend.create_dogs') }}</x-backend.card.title>
    </x-backend.heading>

    <form action="{{ route('backend.dogs.store') }}" method="POST" autocomplete="off">
        @csrf
