        ...
        <x-backend.card.actions>
            @can('delete_users')
                <x-backend.danger-button
                    wire:click="confirm('deleteMany', '{{ __('backend.are_you_sure_you_want_to_delete_the_selected_users?') }}')"
                    :disabled="!count($selectedRecords)">
                    {{ __('backend.delete_users') }}
                </x-backend.danger-button>
            @endcan
            @can('create_users')
                <x-backend.primary-button-link
                    href="{{ route('backend.users.create') }}">{{ __('backend.create_user') }}</x-backend.primary-button-link>
            @endcan
        </x-backend.card.actions>
    </x-backend.heading>
        ...
        <x-backend.table>
            ...
            <x-backend.table.body>
                @forelse($users as $user)
                    <x-backend.table.row wireKey="{{ $user->id }}">
                        @can('delete_users')
                            <x-backend.table.data-cell class="w-10">
                                    @if(!$user->is_admin)
                                        @if($user->id != auth()->id())
                                            <x-backend.checkbox id="ch-{{ $user->id }}" value="{{ $user->id }}"
                                                                wire:model.live="selectedRecords"/>
                                        @endif
                                    @endif
                            </x-backend.table.data-cell>
                        @endcan
                        <x-backend.table.row-header>{{ $user->name }}</x-backend.table.row-header>
                        ...
                        <x-backend.table.data-cell class="flex justify-end gap-3">
                            @can('view_users')
                                <x-backend.anchor href="{{ route('backend.users.show', $user) }}" aria-label="Link to Example.com">
                            ...
                            @can('delete_users')
                                @if(!$user->is_admin)
                                    @if($user->id != auth()->id())
                                        <x-backend.anchor
                                            wire:click="confirm('delete({{ $user->id }})', '{{ __('backend.are_you_sure_you_want_to_delete_this_user?') }}')">
                                            <x-heroicon-o-trash/>
                                        </x-backend.anchor>
                                    @endif
                                @endif
