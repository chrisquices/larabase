    ...
    <x-backend.heading>
        ...
        <x-backend.card.actions>
            ...
            @can('create_permissions')
                <x-backend.primary-button-link href="{{ route('backend.user-management.permissions.create') }}">{{ __('backend.create_permission') }}</x-backend.primary-button-link>
            @endcan
        </x-backend.card.actions>
    </x-backend.heading>
        ...
        <x-backend.table>
            ...
            <x-backend.table.body>
                @forelse($permissions as $permission)
                    <x-backend.table.row wireKey="{{ $permission->id }}" redirectTo="{{ Gate::allows('view_permissions') ? route('backend.user-management.permissions.show', $permission) : false }}">
                        @can('delete_permissions')
                        ...
                        <x-backend.table.data-cell>{{ $permission->name }}</x-backend.table.data-cell>
                        ...
                        <x-backend.table.data-cell class="flex justify-end gap-3">
                            @can('view_permissions')
                                <x-backend.anchor href="{{ route('backend.user-management.permissions.show', $permission) }}">
                                    <x-heroicon-o-magnifying-glass/>
                                </x-backend.anchor>
                            @endcan
                            @can('edit_permissions')
                                <x-backend.anchor href="{{ route('backend.user-management.permissions.edit', $permission) }}">
