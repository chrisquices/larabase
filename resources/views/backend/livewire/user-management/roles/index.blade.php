<div>
    <x-backend.heading>
        ...
        <x-backend.card.actions>
            ...
            @endcan
            @can('create_roles')
                <x-backend.primary-button-link href="{{ route('backend.user-management.roles.create') }}">{{ __('backend.create_role') }}</x-backend.primary-button-link>
            @endcan
        </x-backend.card.actions>
    </x-backend.heading>
        ...
        <x-backend.table>
            ...
            <x-backend.table.body>
                @forelse($roles as $role)
                    <x-backend.table.row wireKey="{{ $role->id }}" redirectTo="{{ Gate::allows('view_roles') ? route('backend.user-management.roles.show', $role) : false }}">
                        @can('delete_roles')
                        ...
                        @endcan
                        <x-backend.table.data-cell>{{ $role->name }}</x-backend.table.data-cell>
                        <x-backend.table.data-cell class="flex justify-end gap-3">
                            @can('view_roles')
                                <x-backend.anchor href="{{ route('backend.user-management.roles.show', $role) }}">
                                    <x-heroicon-o-magnifying-glass/>
                                </x-backend.anchor>
                            @endcan
                            @can('edit_roles')
                                <x-backend.anchor href="{{ route('backend.user-management.roles.edit', $role) }}">
