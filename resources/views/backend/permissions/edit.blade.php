<x-backend.app-layout>
    <x-slot name="title">{{ __('backend.update_permission') }}: {{ $permission->name }}</x-slot>

    <x-backend.heading>
        <x-backend.card.title>{{ __('backend.update_permission') }}: {{ $permission->name }}</x-backend.card.title>
    </x-backend.heading>

    <form action="{{ route('backend.permissions.update', $permission) }}" method="POST" autocomplete="off">
        @csrf
        @method('PATCH')
        ...
        <x-backend.card class="!py-0">
            <x-backend.form.divider>
                <x-backend.form.row>
                    <x-backend.text-label for="name">{{ __('backend.category') }}</x-backend.text-label>
        ...
        <x-backend.form.actions>
            ...
            <x-backend.primary-button>{{ __('backend.update_permission') }}</x-backend.primary-button>
        </x-backend.form.actions>
