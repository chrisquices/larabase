<x-backend.app-layout>
    <x-slot name="title">{{ __('backend.create_user') }}</x-slot>

    <x-backend.heading>
        <x-backend.card.title>{{ __('backend.create_user') }}</x-backend.card.title>
    </x-backend.heading>
        ...
        <x-backend.card class="!py-0">
            <x-backend.form.divider>
                ...
                <x-backend.form.row>
                    ...
                    <x-slot name="input">
                        <x-backend.text-input id="last_name" required/>
                ...
                <x-backend.form.row>
                    ...
                    <x-slot name="input">
                        <x-backend.text-input id="email" type="email" required/>
                    ...
                    <x-slot name="input">
                        <x-backend.select id="locale_id" required>
                            ...
                            @foreach($locales as $locale)
                                <option value="{{ $locale->id }}" @selected(old('locale_id') === $locale->id)>
                                    {{ $locale->code }} ({{ $locale->name }})
                                </option>
                            @endforeach
                        </x-backend.select>
                        <x-backend.input-error name="locale_id"/>
                    </x-slot>
                </x-backend.form.row>
                ...
                <x-backend.form.row>
                    ...
                    <x-slot name="input">
                        <x-backend.select id="is_active" required>
                            <option value="" selected disabled>{{ __('backend.select_an_option') }}</option>
                            <option value="1" @selected(old('is_active') === '1')>{{ __('backend.yes') }}</option>
                            <option value="0" @selected(old('is_active') === '0')>{{ __('backend.no') }}</option>
                        </x-backend.select>
                    ...
                    <x-backend.form.row>
                        ...
                        <x-slot name="input">
                            <x-backend.select id="is_admin" required>
                                <option value="" selected disabled>{{ __('backend.select_an_option') }}</option>
                                <option value="1" @selected(old('is_admin') === '1')>{{ __('backend.yes') }}</option>
                                <option value="0" @selected(old('is_admin') === '0')>{{ __('backend.no') }}</option>
                            </x-backend.select>
                    ...
                    <x-slot name="input">
                        @foreach($roles as $role)
                            <div class="flex items-center gap-2">
                                <x-backend.checkbox id="role_id_{{ $role->id }}" name="role_ids[]" value="{{ $role->id }}"/>
