<x-backend.app-layout>
    ...
    <form action="{{ route('backend.user-management.users.store') }}" method="POST" autocomplete="off">
        @csrf
                    ...
                    <x-backend.form.row>
                        ...
                        <x-slot name="input">
                            <x-backend.select id="is_admin" required>
                                <option value="" selected disabled>{{ __('backend.select_an_option') }}</option>
                                <option value="1" @selected(old('is_admin') === '1')>Yes</option>
                                <option value="0" @selected(old('is_admin') === '0')>No</option>
                            </x-backend.select>
